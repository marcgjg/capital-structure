...
# ─────────── MODEL CONSTANTS ─────────── #
BETA_DECAY  = 2.0      # controls red-curve peak (1/β ≈ 50 %)
FD_EXPONENT = 2.0      # convexity of distress costs
OFFSET      = 7        # spacing (ppt) between the vertical markers
DIST_GAP    = 3        # gap (ppt) between V_L arrow & PV(distress) line
...
opt_d_pct = int(d_pct[opt_idx])         # use integer for clean placement

# x-positions for explanatory arrows
x_left  = max(0,  opt_d_pct - OFFSET)           # PV (tax shield)
x_right = min(100, opt_d_pct + OFFSET)          # Value of levered firm
x_distress = min(100, x_right + DIST_GAP)       # PV(distress) marker

# y-positions where arrows stop
PVTS_top = V_tax[x_left]
VL_top   = V_L[x_right]
VDist_bot, VDist_top = V_L[x_distress], V_tax[x_distress]
...
# grey dotted line → PV (distress costs)
fig.add_shape(type="line",
    x0=x_distress, x1=x_distress,
    y0=VDist_bot, y1=VDist_top,
    line=dict(color="grey", dash="dot"))
fig.add_annotation(
    x=x_distress + 1.5, y=(VDist_bot + VDist_top)/2,
