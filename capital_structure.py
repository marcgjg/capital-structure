...
with col1:
    ...
    try:
        ...
        st.download_button(
            ...
            mime="image/svg+xml",
            help="Vector graphics (editable)"
        )
    except Exception as e:
        st.error("SVG export not available")
        st.info("💡 Use PNG or HTML instead")

with col2:
    # PNG export (reliable fallback)
    try:
        ...
        st.download_button(
            ...
            mime="image/png",
            help="High-quality image"
        )
    except:
        st.info("💡 Use camera icon in chart toolbar for PNG")

with col3:
    # HTML export (always works, perfect reproduction)
    ...
    st.download_button(
        ...
        mime="text/html",
        help="Interactive chart - perfect reproduction"
    )

# Add helpful note
st.info("💡 **Best options:** Use HTML for perfect reproduction, or PNG from the chart's camera icon (top right when you hover over the chart)")
