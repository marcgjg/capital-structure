...
with st.expander("ℹ️ About this tool", expanded=False):
    st.markdown(
        """
        Visualises the classic **tax-shield vs. distress-cost** trade-off.
...
# positions for the two arrows (offset ±1 ppt)
x_PVTS = opt_d_pct - 1
x_VL   = opt_d_pct + 1
...
# dashed arrow → PV tax shield  (left of grey line)
fig.add_shape(type="line",
    x0=x_PVTS, x1=x_PVTS,
    y0=V_U, y1=Vtax_opt,
    line=dict(color="#d62728", dash="dot"))
fig.add_annotation(
    x=x_PVTS - 1.5, y=(V_U + Vtax_opt)/2,
    text="PV (tax shield)",
    showarrow=False, font=dict(size=12, color="#d62728"),
    align="right",
)

# dashed arrow → V_L  (right of grey line)
fig.add_shape(type="line",
    x0=x_VL, x1=x_VL,
    y0=V_U, y1=opt_val,
    line=dict(color="black", dash="dot"))
fig.add_annotation(
    x=x_VL + 1.5, y=(V_U + opt_val)/2,
...
fig.update_layout(
    ...
    margin=dict(l=80, r=80, t=30, b=40),   # y-axis auto-scales
)
